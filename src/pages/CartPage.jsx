...
export function CartPage() {
  ...
  return <section className="page-section cart-page">
    ...
    {items.length ? <div className="cart-layout"><div className="cart-list">{items.map((item) => <div className={`cart-line ${item.customized ? 'cart-line-custom' : ''}`} key={item.id}>
      <div className="cart-line-visual">{item.customized && item.customization?.frontPreview ? <div className="custom-previews"><figure><img src={item.customization.frontPreview} alt={`${item.name} aperçu face`} /><figcaption>Face</figcaption></figure><figure><img src={item.customization.backPreview || item.customization.frontPreview} alt={`${item.name} aperçu dos`} /><figcaption>Dos</figcaption></figure></div> : item.image ? <img src={item.image} alt={item.name} /> : <span className={`product-image-${item.visual}`} />}</div>
